...
class DoseViewer extends React.Component {
    ...
    baseUrl = "https://raw.githubusercontent.com/rrelyea/covid-therapeutics/" + branch + "/data/therapeutics/" + this.props.site + "/dose-history-by-zip/";
        ...
        for (var i = 0; i < this.state.doseInfo.length; i++) {
            ...
            if (provider != null && provider.replaceAll('-',' ').toUpperCase() === this.props.provider.toUpperCase() && reportDate !== null) {
              if (reportDate !== lastReportDate || available !== lastAvailable) {
                ...
                if (this.props.medicine === "Evusheld") {
                  if (dosesAdministered > 0 && dosesAdministered < this.props.dosesInBox && available !== null) {
                  ...
                  {
                    if (dosesAdministered < -(this.props.dosesInBox/2)) {
                      var boxes = Math.ceil(Math.abs(dosesAdministered) / this.props.dosesInBox);
                      var administeredToday = (boxes * this.props.dosesInBox) + dosesAdministered;
